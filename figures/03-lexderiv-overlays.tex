...
\begin{document}
    \begin{tikzpicture}[outer sep=0px, inner sep=1pt, >=stealth]
        ...
        \matrix[below=1cm of tval, xshift=-1cm] (rule) [matrix of nodes, ampersand replacement=\&] {
            \(\alert<2>{S}\) \& \(\to\) \& $[\x_1 \, \text{the} \, \y_1 \, \x_2]$ \& $\begin{bmatrix}\rootclause{}\end{bmatrix}$ \& $(\alert<2>{V_2}, \alert<2>{N})$\\
        };
        ...
        \draw<8,12>[gray] (rule-1-3) edge[|->] (val.south);
                ...
                \node[above=11.5ex of sntresult-1-2] (tval2-0) {SBAR}
                    child {node {S}
                        ...
                        child{node[yshift=-2.5ex] {NP}
                            child[color=black] {node[above=of sntresult-1-2.north]{PT}}
