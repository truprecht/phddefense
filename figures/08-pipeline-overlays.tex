...
\begin{document}
    ...
    \begin{tikzpicture}[font=\small, node distance=1em, remember picture]
        ...
        \node[right=.5cm of extraction,visible on=<2-11>] {\subfile{pipeline/02-rule-lists.tex}};
        ...
        \visible<13->{
            ...
            \node[olabel, font=\tiny, anchor=west, yshift=-1.5cm] (stagmodellab) at (modellab.west) {tokens $\to$ blueprints};
            \node[below=.1cm of stagmodellab, fill=black, obox, rounded corners=2pt] (stagmodel) {\textcolor{lightgray}{S}};
        }
        \visible<14->{
            \node[olabel, font=\tiny, anchor=east, yshift=-.5cm] (posmodellab) at (modellab.east) {tokens $\to$ pos symbols};
        ...
        \visible<4->{
            ...
            \end{scope}
            \node[gray, inner sep=0pt, above=.4cm of term-1-2.north west, anchor=west] (sentence) {sentence};
        ...
        \visible<10->{
            ...
            \begin{scope}[every node/.style={anchor=west, inner sep=0pt, outer sep=0pt}]
                \matrix[right=2cm of term] (stag) [matrix of nodes,nodes in empty cells,row sep=3pt] {
                    ...
                    \staglist[3.5cm]{$V_2 \xrightarrow{0.6} [x_1, \text{5}\, y_1] [\ldots](\mathrm{arg}(V), V\text{-}B)$}\\
                    \staglist[1.7cm]{$V\text{-}B \xrightarrow{0.7} [\text{6}][\ldots]$}\\
                };
            \end{scope}
        ...
        \visible<5->{
            ...
            \node[visible on=<-9>, right=of parserbox, scale=0.5, yshift=-4mm] (derivation) {%
        ...
        \visible<6->{
            ...
            \node[gray, inner sep=0pt, anchor=west] (parselab) at (parse.north west|-sentence) {constituent tree};
        }

        \begin{scope}[->, dashed, gray, >=stealth]
            \foreach \i in {1, ..., 6} {
                \draw<5-9> (term-\i-2) -- (term-\i-2 -| parserbox.west);
                \draw<10-> (term-\i-2) -- (term-\i-2 -| taggerbox.west);
                \draw<10-> (stag-\i-1 -| taggerbox.east)-- (stag-\i-1);
                \draw<10-> (stag-\i-1)-- (stag-\i-1-|parserbox.west);
            }
            \draw<5-9> (grammar.south) -- +(0,-.9) -| coordinate[midway] (grammartoparser) (parserbox);
        ...
        \draw<11> node[red, above=3px of taggerbox] {\Huge?};
    \end{tikzpicture}
\end{document}