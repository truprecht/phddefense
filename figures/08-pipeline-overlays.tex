...
\begin{document}
    ...
    \begin{tikzpicture}[font=\small, node distance=1em, remember picture]
        ...
        \begin{scope}[every path/.style={dashed, gray, ->}, >=stealth]
            ...
            \draw<13-> (mlebox.east|-posmodel) -- (posmodel.west);
        \end{scope}


        \visible<4->{
            \begin{scope}[every node/.style={anchor=west, inner sep=0pt, outer sep=0pt}]
                \matrix[below=2cm of treebank, xshift=-2cm] (term) [matrix of math nodes,nodes in empty cells, row sep=5pt] {
                    \strut \text{where}        \\
                    \strut \text{the}  \\
                    \strut \text{survey}       \\
                    \strut \text{was}\\
        ...
        \begin{scope}[->, dashed, gray, >=stealth]
            \foreach \i in {1, ..., 6} {
                \draw<5-9> (term-\i-1) -- (term-\i-1 -| parserbox.west);
                \draw<10-> (term-\i-1) -- (term-\i-1 -| taggerbox.west);
            ...
            \draw<13-> (stagmodel.east) -- +(1.7,0) -- +(1.7,-2.2) -| (taggerbox);
            \draw<13-> (posmodel.east) -- +(1.5,0) -- +(1.5,-1.7) -| (transbox);
