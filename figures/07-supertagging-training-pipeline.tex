    ...
    \begin{tikzpicture}[font=\small, node distance=1em]
        ...
        \begin{scope}[every node/.style={obox, scale=0.45}]
            \node[right=1cm of extraction, fill=black!40] (rules1) {%
                \begin{tabular}{ll}
                    where & \(V^L \to[\text{*}][\ldots]\)\\
                    the & \(\mathit{Root} \to [x_1\,\text{*}\,y_1\,x_2][\ldots] (V_2, N)\)\\
                    survey& \(N \to [\text{*}] [\ldots]\)\\
                    was& \(V_2 \to [x_1, \text{*}\, x_2] [\ldots] (V_2)\)\\
                    carried& \(V_2 \to [x_1, \text{*}\, y_1] [\ldots](V^L, V^R)\)\\
                    out& \(V^R \to [\text{*}][\ldots]\)
                \end{tabular}%
            };
            \node[below right=.3cm of rules1.north west, anchor=north west, fill=black!20] (rules2) {%
                \begin{tabular}{ll}
                    A & \(N^L \to[\text{*}][\ldots]\)\\
                    hearing & \(N \to [x_1\,\text{*}][\ldots] (N^L)\)\\
                    is & \(\mathit{Root} \to [x_1 \text{*} x_2] [\ldots] (V_2)\)\\
                    scheduled & \(V_2 \to [x_1, \text{*}\, x_2] [\ldots] (V_2)\)\\
                    on & \(V_2 \to [x_1, \text{*}\, y_1] [\ldots](V^L, V^R)\)\\
                    the & \(V^R \to [\text{*}][\ldots]\) \\
                    issue & \(V_2 \to [x_1, \text{*}\, y_1] [\ldots](V^L, V^R)\)\\
                    today & \(V^R \to [\text{*}][\ldots]\)
                \end{tabular}%
            };
            \node[below right=.3cm of rules2.south east, anchor=south east, fill=black!2] (rules3) {%
                \begin{tabular}{ll}
                    What& \(V^L \to[\text{*}] [\ldots]\)\\
                    should& \(S \to [x_1\,\text{*}\,y_1\,x_2][\ldots](V^L, S^R)\)\\
                    I& \(S^R \to [\text{*}][\ldots]\)\\
                    do& \(V_2 \to [x_1\,\text{*}][\ldots] (V^L)\)\\
                    ?& \(\mathit{Root} \to [x_1\,\text{*}][\ldots] (S)\)
                \end{tabular}%
            };
        \end{scope}
        \node[olabel, anchor=west] (ruleslab) at (rules1.west |- treebanklab) {tokens + blueprints\strut};
...
%        \coordinate[right=1cm of mle] (grammaranchor);
        \node[olabel, anchor=north west, align=left, xshift=.9cm] (grammarlab) at (mle.east |- treebanklab.north) {prediction model\strut\\tokens $\to$ blueprints\strut};
        \node[below=.4cm of grammarlab, fill=black, obox, rounded corners=2pt] (grammar1) {\textcolor{lightgray}{S}};
