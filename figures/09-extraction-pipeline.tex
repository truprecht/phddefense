...
\begin{document}
    \begin{tikzpicture}
        ...
        \visible<2->{
            \node[scale=0.8, below=of ctree.south east, anchor=north, xshift=2cm] (bintree) {\subfile{ctrees/04-survey-binarized.tex}};
        }
...
%
        ...
        \begin{scope}[dashed, gray, every node/.style={font=\scriptsize, align=left}, >=stealth]
            \draw[visible on=<2->] (ctree.-100) edge[bend right, ->] node[left, alarm on=<17|handout:0>] {rank transformation:\\$\bullet$\,type (binarization, head-inward)\\$\bullet$\,Markovzation} (bintree.west);
