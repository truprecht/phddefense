...
\begin{document}
    \begin{tikzpicture}[
        ...
        \node[above=22.5ex of pos-1-3, xshift=-2em, alarm on=<8>] (sbar) {SBAR}
        child { node[alarm on=<8>] (s) {S}
            [sibling distance=2em]
            child { node[alarm on=<8>] (vp1) {VP}
                child { node (vp2) {VP}
                    child { node[above=4.5ex of pos-1-1, alarm on=<7>] (wh) {WH} child {
                            node[alarm on=<7>] (term0) {1}}}
                    child {
                        node[above=0ex of pos-1-5] (term4) {5}}
                    child { node[above=4.5ex of pos-1-6] (prt) {PRT} child {
                            node (term5) {6}}}}
                child {
                    node[above=0ex of pos-1-4, alarm on=<8-10>] (term3) {4}}}
