...
\begin{document}
    \begin{frame}<1-12>[label=extractionpipe]{Lexicalized Rule Extraction}
        \resizebox{\linewidth}{!}{%
        \begin{tikzpicture}
            \node[scale=0.8] (ctree) {\subfile{figures/ctrees/01-survey.tex}};

            \visible<2>{
                \node[scale=0.8, below=of ctree.south east, anchor=north, xshift=1cm] {
                    \subfile{figures/ctrees/04-survey-binarized.tex}
                };
            }
            ...
            \begin{scope}[dashed, gray, every node/.style={font=\small, text=black, align=left, draw=gray, rectangle, rounded corners=2pt, solid}, >=stealth]
                ...
                \draw[visible on=<5->] ($(bintree)+(1,1)$) edge[bend right, ->] node[above left] {assign guide values:\\\alert<12|handout:0>{$\bullet$\,guide constructor}} (guidetree);
